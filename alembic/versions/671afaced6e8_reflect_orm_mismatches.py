...
def upgrade() -> None:
    op.alter_column('cv_data', 'voltage_input',
               existing_type=mysql.DECIMAL(precision=10, scale=5),
               nullable=False)
    op.alter_column('cv_data', 'capacitance',
    ...
    op.alter_column('iv_data', 'chip_id',
               existing_type=mysql.INTEGER(),
               nullable=False)
    op.alter_column('iv_data', 'voltage_input',
               existing_type=mysql.DECIMAL(precision=10, scale=5),
               nullable=False)
    op.alter_column('iv_data', 'anode_current',
               existing_type=mysql.FLOAT(),
               nullable=False)
    op.alter_column('wafer', 'name',
               existing_type=mysql.VARCHAR(length=20),
               nullable=False)
    op.alter_column('chip', 'wafer_id',
               existing_type=mysql.INTEGER(),
               nullable=False)
...
def downgrade() -> None:
    ...
    op.create_foreign_key('cv_data_FK_1', 'cv_data', 'chip', ['chip_id'], ['id'], onupdate='RESTRICT', ondelete='RESTRICT')
    op.create_foreign_key('cv_data_FK', 'cv_data', 'chip_state', ['chip_state_id'], ['id'], onupdate='RESTRICT', ondelete='RESTRICT')
