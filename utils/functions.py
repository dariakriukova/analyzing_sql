...
def select_one(
        items: list[T], prompt: str, id_name_getter: Callable[[T], (int, str)] = None) -> T:
    if id_name_getter is None:
        id_name_getter = lambda x: (x.id, x.name)
    ...
    selected_id = click.prompt(
        f"{prompt}\n{option_help}", type=option_type, show_choices=False, prompt_suffix='\n')
